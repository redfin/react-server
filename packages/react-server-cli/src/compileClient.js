...
export default (opts = {}) => {
	...
	const serverWebpackConfig = webpackConfigFunc(packageCodeForNode(serverEntrypoints, serverOutputDirAbsolute, outputUrl, false, false, true, stats));
	const compiler = webpack([clientWebpackConfig, serverWebpackConfig]);

	const serverRoutes = new Promise((resolve) => {
		compiler.plugin("done", (stats) => {
			console.log(stats);
			const clientManifest = statsToManifest(stats.stats[0]);
			const serverManifest = statsToManifest(stats.stats[1]);
			fs.writeFileSync(path.join(outputDir, "manifest.json"), JSON.stringify(clientManifest));
			fs.writeFileSync(path.join(serverOutputDir, "manifest.json"), JSON.stringify(serverManifest));
			...
			if (!hot) {
				...
				fs.unlinkSync(path.join(serverOutputDir, "chunk-manifest.json"));
			}

			resolve(writeWebpackCompatibleRoutesFile(routes, routesDir, serverWorkingDirAbsolute, outputUrl, false, clientManifest));
		});
	});
